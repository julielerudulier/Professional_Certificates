...
def binary_search_year(lst, year):
    lst.sort()
    ...
    if lst[middle].year == year:
        return True

    elif year < lst[middle].year:
        return binary_search_year(lst[:middle], year)

    else:
        return binary_search_year(lst[middle + 1:], year)
