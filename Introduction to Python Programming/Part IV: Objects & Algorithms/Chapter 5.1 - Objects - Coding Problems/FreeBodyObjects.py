...
class Force:
    ...
    def get_horizontal(horizontal):
        horizontal = horizontal.magnitude * cos(radians(horizontal.angle))
        return horizontal
    
    def get_vertical(vertical):
        vertical = vertical.magnitude * sin(radians(vertical.angle))
        return vertical
    
    def get_angle(result, use_degrees = True):
        if use_degrees:
            return result.angle
        else:
            return radians(result.angle)
